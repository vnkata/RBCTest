...
public class DeclsExit {
    ...
    public String generateDtrace(TestCase testCase, DeclsEnter declsEnter) {
        ...
        if (JSONManager.isStringJsonArray(responseBody)) {
            ...
            if (this.isNestedArray) { // (Bad practice) If the response is parseable to array and the exit is of type
                                      // nestedArray
                ...
                int targetNestingLevel = (int) Arrays
                        ...
                        .count();
                // Count the number of arrays corresponding to the number of .arrays and return
                // the dtrace
                try {
            ...
            } else { // (Bad practice) If the response is parseable to array but the exit is not of
                     // type nestedArray (i.e., we are generating a dtrace for one of its elements)
                // Remove all the arrays until reaching an object and continue with normal
                // behaviour
    ...
    private String generateSingleDtraceEnterAndExit(List<JSONObject> jsonObjectList, TestCase testCase,
            DeclsEnter declsEnter) {
        StringBuilder res = new StringBuilder();
        ...
        for (JSONObject json : jsonObjectList) {
            ...
            // The elements of this list can be either JSONArray (If there are elements with
            // nested arrays) or JSONObject
            ...
            for (Object jsonElement : nestedJsonObjects) {
                ...
                if (jsonElement instanceof JSONObject) { // If the element is of type JSONObject
                    ...
                    res.append(
                            this.generateSingleDtraceEnterAndExitArray((JSONArray) jsonElement, testCase, declsEnter));
                }
    ...
    private String generateSingleDtraceEnterAndExitArray(JSONArray jsonArray, TestCase testCase,
            DeclsEnter declsEnter) {
     ...
     * @param testCase
     * @param jsonArray
     * @return A string representing the dtrace of an array of JSON Objects. If the
     *         value of the array is null, its hashcode is null and its value is set
     *         to nonsensical. Example:
     *         return.markets
     *         null
     *         1
     *         return.markets[..]
    ...
    public String generateSingleDtraceExitArray(TestCase testCase, JSONArray jsonArray) {
        ...
        String value = generateDtraceExitValueOfJSONArray(testCase, jsonArray,
                this.exitDeclsVariables.getEnclosedVariables().get(0).getDecType(), "array");
        ...
        // Group 3
        res = res + "return.array[..]" + "\n";
        // The declaration type of the first enclosed variable will always reveal the
