...
public class TestCase {
    ...
    private static String propertiesFilePath = "spec/GitLabProject/gitlab.properties";
    // "src/test/resources/Stripe/stripe_createCoupon.properties";
    ...
    private static Boolean deletePreviousResults; // Set to 'true' if you want pre vious CSVs and Allure reports.
    ...
    private static boolean checkTestCases; // If 'true', test cases will be checked with OASValidator before executing
                                           // them
    ...
    public static void main(String[] args) throws RESTestException {
        ...
        // Read .properties file path. This file contains the configuration parameters
        // for the generation
        if (args.length > 0)
            propertiesFilePath = args[0];

        // Populate configuration parameters, either from arguments or from .properties
        // file
        ...
        RESTestWorkflow runner = new RESTestWorkflow(testClassName, targetDirJava, packageName, spec, confPath,
                generator, writer,
                reportManager, statsReportManager);

        runner.setExecuteTestCases(false);
        ...
        while (totalNumTestCases == -1 || runner.getNumTestCases() < totalNumTestCases) {
            ...
            // Generate unique test class name to avoid the same class being loaded
            // everytime
            // String id = IDGenerator.generateTimeId();
            String id = "1751198680788";
    ...
    private static IWriter createWriter() {
        ...
        RESTAssuredWriter writer = new RESTAssuredWriter(OAISpecPath, confPath, targetDirJava, testClassName,
                packageName,
                basePath, logToFile);
    ...
    private static StatsReportManager createStatsReportManager() {
        ...
        CoverageMeter coverageMeter = enableInputCoverage || enableOutputCoverage
                ? new CoverageMeter(new CoverageGatherer(spec))
                : null;
    ...
    private static void generateTimeReport(Integer iterations) {
        String timePath = readParameterValue("data.tests.dir") + "/" + experimentName + "/"
                + readParameterValue("data.tests.time");
        try {
    ...
    // Read the parameter value from: 1) CLI; 2) the local .properties file; 3) the
    // global .properties file (config.properties)
    private static String readParameterValue(String propertyName) {
        ...
        if (argsList.contains(propertyName))
            ...
            value = argsList.stream().filter(arg -> arg.matches("^" + propertyName + "=.*")).findFirst().get()
                    .split("=")[1];
        else if (PropertyManager.readProperty(propertiesFilePath, propertyName) != null) // Read value from local
                                                                                         // .properties file
    ...
    private static void setUpLogger() {
        ...
        String logPath = readParameterValue("data.log.dir") + "/" + readParameterValue("experiment.name") + "/"
                + readParameterValue("data.log.file");
