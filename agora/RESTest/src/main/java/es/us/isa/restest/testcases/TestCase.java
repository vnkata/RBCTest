...
public class TestCase implements Serializable {
	...
	private String faultyReason; // "none", "individual_parameter_constraint", "invalid_request_body" or
									// "inter_parameter_dependency"
	private Boolean enableOracles; // True (default) if you want to assert the response. False if you only want to
									// execute the request
	...
	/**
	 * WARNING: Empty parameters cannot be used in path. If this happens, replace by
	 * "null"
	 */
	...
	public void exportToCSV(String filePath) {
		if (!checkIfExists(filePath)) // If the file doesn't exist, create it (only once)
			createCSVwithHeader(filePath,
					"testCaseId,faulty,faultyReason,fulfillsDependencies,operationId,path,httpMethod,inputContentType,outputContentType,"
							+
							"headerParameters,pathParameters,queryParameters,formParameters,bodyParameter");

		// Generate row, we need to escape all fields susceptible to contain characters
		// such as ',', '\n', '"', etc.
		String rowBeginning = id + "," + faulty + "," + escapeCsv(faultyReason) + "," + fulfillsDependencies + ","
				+ operationId + "," + path + "," + method.toString() + "," + inputFormat + "," + outputFormat + ",";
		StringBuilder rowEnding = new StringBuilder();
		try {
			for (Map.Entry<String, String> h : headerParameters.entrySet()) {
				rowEnding.append(encode(h.getKey(), StandardCharsets.UTF_8.toString())).append("=")
						.append(encode(h.getValue(), StandardCharsets.UTF_8.toString())).append(";");
			}
			rowEnding.append(",");
			for (Map.Entry<String, String> p : pathParameters.entrySet()) {
				rowEnding.append(encode(p.getKey(), StandardCharsets.UTF_8.toString())).append("=")
						.append(encode(p.getValue(), StandardCharsets.UTF_8.toString())).append(";");
			}
			rowEnding.append(",");
			for (Map.Entry<String, String> q : queryParameters.entrySet()) {
				rowEnding.append(encode(q.getKey(), StandardCharsets.UTF_8.toString())).append("=")
						.append(encode(q.getValue(), StandardCharsets.UTF_8.toString())).append(";");
			}
			rowEnding.append(",");
			for (Map.Entry<String, String> f : formParameters.entrySet()) {
				rowEnding.append(encode(f.getKey(), StandardCharsets.UTF_8.toString())).append("=")
						.append(encode(f.getValue(), StandardCharsets.UTF_8.toString())).append(";");
			}
		...
		if (this.getFormParameters().size() > 0) {
			...
			try {
				for (Map.Entry<String, String> formParam : this.getFormParameters().entrySet()) {
					formDataBody.append(encode(formParam.getKey(), StandardCharsets.UTF_8.toString())).append("=")
							.append(encode(formParam.getValue(), StandardCharsets.UTF_8.toString())).append("&");
				}
			} catch (UnsupportedEncodingException e) {
				LogManager.getLogger(TestCase.class.getName())
						.warn("Parameters of test case could not be encoded. Stack trace:");
	...
	/**
	 * Returns true if the test case is valid according to the specification, false
	 * otherwise.
	 * 
	...
	/**
	 * Returns true if the test case fulfills inter-parameter dependencies, false
	 * otherwise
	 * 
	 * @param tc          a test case
	 * @param idlReasoner the IDLReasoner analyzer
	 * @return true if the test case fulfills inter-parameter dependencies, false
	 *         otherwise
	 */
	public static Boolean checkFulfillsDependencies(TestCase tc, Analyzer idlReasoner) {
		if (idlReasoner == null)
			return true;
		try {
			return idlReasoner.isValidRequest(restest2idlTestCase(tc)); // Previous version of IDLReasoner:
																		// idlReasoner.isValidRequest(restest2idlTestCase(tc),
																		// true);
