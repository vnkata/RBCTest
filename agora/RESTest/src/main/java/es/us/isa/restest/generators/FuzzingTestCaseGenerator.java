...
/**
 * This class implements a generator of fuzzing test cases. It uses a
 * customizable dictionary to obtain
 * fuzzing parameters. Those parameters are classified by type (string, integer,
 * number, boolean).
 *
...
public class FuzzingTestCaseGenerator extends AbstractTestCaseGenerator {
            ...
            for (TestParameter testParam : testOperation.getTestParameters()) {
                if (!testParam.getIn().equals("body")) {
                    OpenAPIParameter param = OpenAPISpecificationVisitor.findParameterFeatures(
                            testOperation.getOpenApiOperation(), testParam.getName(), testParam.getIn());
                    ...
                    nominalGenerators.replace(Pair.with(testParam.getName(), testParam.getIn()),
                            Arrays.asList(generator, commonFuzzingGenerator));
                }
            }
        }
        ...
        if (testOperation.getTestParameters() != null) {
            for (TestParameter testParam : testOperation.getTestParameters()) {
                if (testParam.getWeight() == null || rand.nextFloat() <= testParam.getWeight()) {
                    if (!testParam.getIn().equals("body")) {
                        tc.addParameter(testParam,
                                nominalGenerators.get(Pair.with(testParam.getName(), testParam.getIn()))
                                        .get(rand.nextInt(2)).nextValueAsString());
                    } else {
    ...
    private void generateFuzzingBody(TestCase tc, TestParameter testParam, Operation testOperation) {
        Map.Entry<String, MediaType> mediaTypeEntry = testOperation.getOpenApiOperation().getRequestBody().getContent()
                .entrySet()
    ...
    private void generateFuzzingBody(Schema schema, JsonNode rootNode, List<String> requiredProperties) {
        ...
        for (Object o : entries) {
            ...
            if (requiredProperties == null && rootNode.isArray() // Array has no req. properties, but generate at least
                                                                 // one
                    || (requiredProperties != null && requiredProperties.contains(entry.getKey())) // Req. property
                    || ((requiredProperties == null || !requiredProperties.contains(entry.getKey()))
                            && rand.nextBoolean())) { // Optional property (50% prob.)
