...
public abstract class AbstractTestCaseGenerator {
	...
	// The following pairs stand for Pair<ParameterName,Type(query, path,
	// header...)>
	protected Map<Pair<String, String>, List<ITestDataGenerator>> nominalGenerators; // Nominal test data generators
																						// (random, boundaryValue,
																						// fixedlist...)
	protected Map<Pair<String, String>, List<ITestDataGenerator>> faultyGenerators; // Faulty test data generators
																					// (random, boundaryValue,
																					// fixedlist...)
	protected AuthManager authManager; // For if multiple API keys are used for the API
	protected Float faultyRatio = 0f; // Ratio (0-1) of faulty test cases to be generated on each operation. Defaults
										// to 0.1
	protected int numberOfTests; // Number of test cases to be generated for each operation
	private int maxTriesPerTestCase = 100; // Maximum number of tries for generating a random test case conforming the
											// input OAS schema.
	 ...
	 * <li>Each required parameter in the OAS must exist in the testConf and it must
	 * have a weight of "null" or "1"</li>
	 * </ol>
	 *
	...
	private void preconditions(TestConfigurationObject conf) {
		...
		for (Operation testOperation : conf.getTestConfiguration().getOperations()) {
			List<OpenAPIParameter> requiredParams = OpenAPISpecificationVisitor
					.getRequiredParametersFeatures(testOperation.getOpenApiOperation());

			if (testOperation.getTestParameters() != null) {

				for (TestParameter testParameter : testOperation.getTestParameters()) {

					OpenAPIParameter param = OpenAPISpecificationVisitor.findParameterFeatures(
							testOperation.getOpenApiOperation(), testParameter.getName(), testParameter.getIn());
					if (param == null) {
						throw new IllegalArgumentException(
								"Each parameter in the testConf must exist in the OAS; unknown parameter: "
										+ testParameter.getName() + ", in: " + testParameter.getIn());
					}

					if (requiredParams.contains(param) && testParameter.getWeight() != null
							&& !testParameter.getWeight().equals(1F)) {
						throw new IllegalArgumentException(
								"Each required parameter in the OAS must exist in the testConf and it must have a weight of 'null' or '1'; parameter: "
										+ testParameter.getName() + ", in: " + testParameter.getIn() + ", weight: "
										+ testParameter.getWeight());
					}
				}
			}
		}
	 ...
	 * @throws RESTestException if the test case generated does not conform to the
	 *                          specification
	 */
	...
	/**
	 * Generate a set of test cases for the whole configuration file (all paths, all
	 * operations)
	 * 
	 * @return Generated test cases (duplicates are possible)
	 * @throws RESTestException if the test HTTP method is other than 'get', 'post',
	 *                          'put' or 'delete'
	 */
	public Collection<TestCase> generate() throws RESTestException {
		...
		for (Operation testOperation : conf.getTestConfiguration().getOperations()) {
			TestConfigurationFilter filter = filters.stream()
					.filter(x -> x.getPath().equalsIgnoreCase(testOperation.getTestPath())).findFirst().orElse(null);
			int filterIndex = -1;
	...
	/**
	 * Generate the next test case and update the generation index. To be
	 * implemented on each subclass.
	 * This method MUST call the {@link #checkTestCaseValidity(TestCase)} method
	 * before returning the test case.
	 */
	...
	/*
	 * Generate a basic valid random test case according to the test configuration
	 * file in two steps:
	 * 1) Select the parameters to be included in the test case based on their
	 * weight.
	 * 2) Generate valid test values with the corresponding test data generators in
	 * the test configuration file. Input objects (if any) are not perturbated yet
	 * 3) Try to perturbate input objects (if any). If no valid perturbation can be
	 * generated, set the parameter to the original object provided in the test
	 * configuration file
	 */
	protected TestCase generateRandomValidTestCase(Operation testOperation) throws RESTestException {
		...
		if (testOperation.getTestParameters() != null) {
			for (TestParameter confParam : testOperation.getTestParameters()) {
				// If the parameter is not required, check if it should be included in the test
				// cases
				if (confParam.getWeight() == null || rand.nextFloat() <= confParam.getWeight()) {
					ITestDataGenerator generator = getRandomGenerator(
							nominalGenerators.get(Pair.with(confParam.getName(), confParam.getIn())));
					if (generator instanceof ObjectPerturbator) {

						test.addParameter(confParam, ((ObjectPerturbator) generator).getRandomOriginalStringObject()); // Objects
																														// are
																														// not
																														// perturbated
																														// yet
						...
						test.addParameter(confParam, ((BodyGenerator) generator).nextValueAsString(false)); // Objects
																											// are not
																											// mutated
																											// yet
					} else
		...
		// If a perturbation generator is included in the test configuration file, try
		// to generate a new (valid) test case by perturbating a valid input object
		if (perturbation)
	...
	/**
	 * Make sure the test case generated conforms to the specification. Otherwise,
	 * throw an exception and stop the execution
	 * There's an exception: if stateful generators are configured we cannot assure
	 * that the test case
	...
	/**
	 * Tries to make a test case faulty by using an invalid generator for a random
	 * parameter
	 * 
	...
	protected boolean makeTestCaseFaultyDueToInvalidGenerator(TestCase testCase, Operation testOperation) {
		...
		// Get random parameter (and faulty generator) which will contain an invalid
		// value
		Object[] possibleFaultyParams = faultyGenerators.keySet().toArray();
		Pair<String, String> faultyParam = (Pair<String, String>) possibleFaultyParams[rand
				.nextInt(possibleFaultyParams.length)];
		...
		testCase.setFaultyReason(INDIVIDUAL_PARAMETER_CONSTRAINT + ":Set parameter " + faultyParam.getValue0()
				+ " with invalid value " + faultyValue);

		return true;
	}

	/**
	 * Tries to make a test case faulty by violating an individual constraint (ex.
	 * excluding a required parameter)
	 * 
	 * @param testCase      The test case to modify
	 * @param testOperation The operation the test case refers to
	 * @return {@code true} if the test case was modified, {@code false} otherwise
	 ...
	 */
	protected boolean makeTestCaseFaultyDueToIndividualConstraints(TestCase testCase, Operation testOperation)
			throws RESTestException {
		...
		if (testOperation.getOpenApiOperation().getRequestBody() != null) {
			TestParameter bodyParam = testOperation.getTestParameters().stream().filter(x -> x.getName().equals("body"))
		...
		while (i < generationAlternatives.size() && originalTest.equals(testCase)) {
			...
			if (originalTest.equals(testCase) && generationAlternatives.get(i).equals("request_body_mutation")) { // Test
																													// case
																													// with
																													// invalid
																													// body
	...
	/**
	 * Tries to make a test case faulty by generating an invalid body. Works only if
	 * there's a body and a
	 ...
	 * @throws RESTestException
	 */
	protected boolean makeTestCaseFaultyDueToInvalidRequestBody(TestCase testCase, Operation testOperation,
			ITestDataGenerator bodyGenerator) throws RESTestException {
		TestCase originalTest = new TestCase(testCase);
		TestParameter bodyParam = testOperation.getTestParameters().stream().filter(x -> x.getName().equals("body"))
				.findFirst().orElse(null);
		...
		if (bodyGenerator != null && bodyParam != null) {
			...
			// No invalid body generated. Return null and try to generate faulty test case
			// in different way
			if (errors.isEmpty()) {
				logger.warn(
						"Maximum number of tries reached when trying to generate invalid JSON body for the operation {}",
						testOperation.getOpenApiOperation().getOperationId());
	...
	/*
	 * Try to perturbate input objects using the ObjectPerturbator. If not possible,
	 * set the parameter to the original object provided in the test configuration
	 * file
	 */
		...
		for (TestParameter confParam : testOperation.getTestParameters()) {
			...
			if (generator instanceof ObjectPerturbator) {
				...
				if (!valid) {
					...
					logger.warn(
							"Maximum number of tries reached when trying to perturbate the input object for the operation {}",
							testOperation.getOpenApiOperation().getOperationId());
				}
			}
		}
	...
	// Returns true if there are more test cases to be generated. To be implemented
	// on each subclass.
	protected abstract boolean hasNext();

	// Create an empty test case with a random name.
	protected TestCase createTestCaseTemplate(Operation testOperation) {
		String testId = "test_" + IDGenerator.generateId() + "_"
				+ removeNotAlfanumericCharacters(testOperation.getOperationId());
		TestCase test = new TestCase(testId, false, testOperation.getOperationId(), testOperation.getTestPath(),
				HttpMethod.valueOf(testOperation.getMethod().toUpperCase()));
			...
			for (TestParameter param : testParameters) {
				...
				for (Generator g : param.getGenerators()) {
					...
					if (gen instanceof BodyGenerator) {
						((BodyGenerator) gen)
								.setDataDirPath(spec.getPath().substring(0, spec.getPath().lastIndexOf('/')));
					...
					if (gen instanceof ParameterGenerator) {
						...
						((ParameterGenerator) gen).setParameterType(OpenAPISpecificationVisitor
								.findParameterFeatures(operation.getOpenApiOperation(), param.getName(), param.getIn())
								.getType());
					}
