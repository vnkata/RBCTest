...
public class DefaultTestConfigurationGenerator {
	 ...
	 * 
	 * @param destination Path of the output test configuration file
	 * @param filters     Filters setting the paths and HTTP methods to be included
	 *                    in the test
	...
	// Generate the test configuration data for the target paths and operations (as
	// indicated in the filters)
		...
		switch (paramType) {
			...
			default:
				throw new IllegalArgumentException(
						"The parameter type " + paramType + " is not allowed in query or path");
		}
	}
	...
	private List<TestParameter> generateRequestBodyTestParameters(RequestBody requestBody) {
		...
		if (requestBody.getContent().keySet().stream().anyMatch(x -> x.matches(MEDIA_TYPE_APPLICATION_JSON_REGEX)) ||
			...
			if (requestBody.getContent().keySet().stream()
					.anyMatch(x -> x.matches(MEDIA_TYPE_APPLICATION_JSON_REGEX))) {
				Entry<String, MediaType> mediaTypeEntry = requestBody.getContent().entrySet().stream()
						.filter(x -> x.getKey().matches(MEDIA_TYPE_APPLICATION_JSON_REGEX)).findFirst().orElse(null);
				if (mediaTypeEntry != null)
					addObjectPerturbatorIfPossible(gens, mediaTypeEntry.getValue());

				// Regardless of whether an ObjectPerturbator generator was set or not, add a
				// BodyGenerator:
			...
			// logger.warn("No properties defined for media type: " +
			// mediaType.getSchema().getType());
			// return testParameters;
			// // return testParameters; // No parameters to generate
			// }
