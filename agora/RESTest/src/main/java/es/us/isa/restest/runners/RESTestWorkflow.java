...
/**
 * This class implements a basic test workflow: test generation -> test writing
 * -> class compilation and loading -> test execution -> test report generation
...
public class RESTestWorkflow {
	...
	public RESTestWorkflow(String testClassName, String targetDir, String packageName, OpenAPISpecification spec,
			String confPath, AbstractTestCaseGenerator generator, IWriter writer, AllureReportManager reportManager,
	...
	protected void testExecution(Class<?> testClass) {
		...
		logger.info("{} tests run in {} seconds. Successful: {}, Failures: {}, Ignored: {}", result.getRunCount(),
				result.getRunTime() / 1000, successfulTests, result.getFailureCount(), result.getIgnoreCount());
