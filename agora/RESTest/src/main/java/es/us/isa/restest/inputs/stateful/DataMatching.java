...
/**
 * Utility class used by BodyGenerator and ParameterGenerator. Given a
 * dictionary
...
public class DataMatching {
    ...
    public static JsonNode getParameterValue(ObjectNode dict, String operationMethod, String operationPath,
            String paramName) {
        ...
        if ("id".equalsIgnoreCase(paramName)) {
            paramValue = getParameterValue(dict, operationMethod, operationPath,
                    getIdParameterName(paramName, operationPath));
            if (paramValue != null)
                return paramValue;
        }
        ...
        // 5th option: Repeat whole process with sub-property name (e.g.,
    ...
    private static String getIdParameterName(String paramName, String operationPath) {
        String lastResourceOperationPath = operationPath.replaceAll("(/\\{[^/]*\\})+$", "");
        return m.stem(lastResourceOperationPath
                .substring(lastResourceOperationPath.lastIndexOf('/') + 1)
                .replaceAll(
                        "^([gG]et|[sS]et|[pP]ost|[pP]ut|[dD]elete|[pP]atch|[oO]btain|[rR]etrieve|[cC]reate|[uU]pdate|[rR]emove)([A-Z])",
                        "$2"))
