...
public class BodyGenerator implements ITestDataGenerator {
    ...
    public JsonNode nextValue() {
        ...
        if (!checkIfExists(jsonPath) && defaultValue != null) {
            try {
                ...
                logger.warn("The defaultValue used for the body of {}{} is not a valid JSON", operationMethod,
                        operationPath);
            }
        }

        ObjectNode dictNode = operationPath != null && FileManager.checkIfExists(jsonPath)
                ? (ObjectNode) JSONManager.readJSON(jsonPath)
        ...
        if (requestBody != null) {
            Schema mutatedSchema = mutate
                    ? new SchemaMutation(requestBody.getSchema(), spec.getSpecification()).mutate()
            ...
            try {
                ...
                logger.warn("There isn't enough data to generate a valid request body for {} operation.",
    ...
    private void generateStatefulObjectNode(ObjectNode dictNode, Schema<?> schema, JsonNode rootNode, String prefix,
            List<String> requiredProperties, boolean firstLevel) throws RESTestException {
        if (schema.get$ref() != null) {
            schema = spec.getSpecification().getComponents().getSchemas()
                    .get(schema.get$ref().substring(schema.get$ref().lastIndexOf('/') + 1));
        }

        String resolvedProperty = prefix.substring(prefix.lastIndexOf('.') + 1).replace("-duplicated", "")
                .replace(DOT_CONVERSION, ".");

        if ("".equals(prefix) // First level object
                || requiredProperties == null && rootNode.isArray() // Array has no req. properties, but generate at
                                                                    // least one
                ...
                || ((requiredProperties == null || !requiredProperties.contains(resolvedProperty))
                        && random.nextBoolean())) { // Optional property (50% prob.)
            JsonNode childNode = null;
            if (schema.getType() == null) {
                logger.warn("The schema of {}{} has no type defined. RESTest will use a default value.",
                        operationMethod, operationPath);
            }
            ...
            if (!"".equals(prefix) || !firstLevel) {
                if (childNode == null || childNode.isMissingNode()) {
                    ...
                    ((ObjectNode) rootNode)
                            .set(prefix.substring(prefix.lastIndexOf('.') + 1).replace(DOT_CONVERSION, "."), childNode);
                } else {
    ...
    private JsonNode createNodeFromExample(Schema<?> schema, String prefix) {
        ...
        if (schema.getExample() != null) {
            ...
            if (requestBody.getExamples() != null) {
                example = ((Example) requestBody.getExamples().values().toArray()[random
                        .nextInt(requestBody.getExamples().values().size())]).getValue();
            ...
            if (example != null) {
                ...
                while (i < prefixSplit.length) {
                    if (exampleNode.get(prefixSplit[i]) != null) {
                        candidate = exampleNode.get(prefixSplit[i]).isArray() ? exampleNode.get(prefixSplit[i]).get(0)
                                : exampleNode.get(prefixSplit[i]);
