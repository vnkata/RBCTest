...
public class NestedPpts {
    ...
    public static List<DeclsExit> getAllNestedDeclsExits(String endpoint, String operationName,
            String variableNameInput,
        ...
        if (parameterType != null && parameterType.equalsIgnoreCase(ARRAY_TYPE_NAME)) {
            ...
            while (parameterType.equalsIgnoreCase(ARRAY_TYPE_NAME)) {
                DeclsExit declsExit = new DeclsExit(endpoint, operationName, variableNameInput, enterVariables,
                        outputObjectName,
        ...
        // If there is an allOf, parameterType is null, but the schema contains all the
        // properties
        if (parameterType == null || parameterType.equalsIgnoreCase(OBJECT_TYPE_NAME)) {
            ...
            for (String nameSuffix : allSchemas.keySet()) {
                // if (allSchemas.get(nameSuffix).getProperties() == null) { // If the element
                // is of type array, call the
                // constructor that receives an ArraySchema
                if (allSchemas.get(nameSuffix).getType() == "array") { // If the element is of type array, call the
                    System.out.println(allSchemas.get(nameSuffix));
                    ...
                    res.add(declsExit);
                } else { // If the element is of type object, call the constructor that receives an
                         // Schema
                    DeclsExit declsExit = new DeclsExit(endpoint, operationName,
                            variableNameInput, enterVariables, outputObjectName, allSchemas.get(nameSuffix), nameSuffix,
                            statusCode);
                    res.add(declsExit);
                }
