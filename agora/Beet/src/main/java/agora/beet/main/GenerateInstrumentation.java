...
public class GenerateInstrumentation {
    ...
    private static String openApiSpecPath = "src/main/resources/Data_2xx_50/GitLab Branch/spec.json";
    private static String testCasesFilePath = "src/main/resources/Data_2xx_50/GitLab Branch/post+projects_{id}_repository_branches/post+projects_{id}_repository_branches.csv";
    ...
    public static final List<String> primitiveTypes = Arrays.asList(STRING_TYPE_NAME, DOUBLE_TYPE_NAME,
            INTEGER_TYPE_NAME, BOOLEAN_TYPE_NAME);
    ...
    public static void main(String[] args) {
        ...
        if (generateDtrace) {
            int i = 0;
            String dtraceFilePath = getOutputPath("dtraceFile.dtrace", testCasesFilePath); // openApiSpecPath
                                                                                           // testCasesFilePath
            ...
            try {
                ...
                while ((testCasesLine = testCasesBR.readLine()) != null) {
                    ...
                    i++;
                    System.out.println(i + " test cases");
                    ...
                    for (DeclsClass declsClass : declsFile.getClasses()) {
                        ...
                        if (declsClass.getClassName().equalsIgnoreCase(testCase.getPath())) {
                            System.out.println("Processing test case: " + testCase.getPath() + " "
                                    + declsClass.getClassName() + " with status code: "
                            ...
                            // System.out.println(declsExits.size() + " exits found for status code: "
                            // + testCase.getStatusCode());
                            for (DeclsExit declsExit : declsExits) {
                                ...
                                DeclsEnter declsEnter = declsClass.getDeclsEnters().stream()
                                        .filter(x -> x.getStatusCode().equals(declsExit.getStatusCode())
                                                && x.getNameSuffix().equals(declsExit.getNameSuffix()))
                                        .findFirst().orElseThrow(() -> new NullPointerException(
                                                "Could not find the corresponding DeclsEnter"));
    ...
    public static String getOperationName(Operation operation, Entry<HttpMethod, Operation> operationEntry,
